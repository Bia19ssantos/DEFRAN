        ...
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                try {
                    ...
                    console.log(clienteDefran); // Verifique se os dados estão corretos
                    carregarTabelaClientes(clienteDefran, filtro);
                } catch (error) {
...
function carregarTabelaClientes(clienteDefran, filtroCliente) {
    var tableBody = document.getElementById("table-body-clientes");
    tableBody.innerHTML = ""; // Limpar a tabela antes de popular novamente

    clienteDefran.forEach(cliente => {
        if (cliente.razao.toLowerCase().includes(filtroCliente.toLowerCase())) {
            var row = tableBody.insertRow();
            row.setAttribute("data-id", cliente); // Adicione o atributo data-id com o índice
            ...
            cellRazao.textContent = cliente.razao;
...
document.addEventListener("DOMContentLoaded", function () {
    var inputFiltro = document.getElementById("filtroCliente");
